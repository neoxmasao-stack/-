...
check "L5" "21-country official index exists" "$(required_file docs/19_license_register_index_21_countries.md && echo 1 || echo 0)"
...
check "L9" "Official-source-only policy is documented" "$(contains_text docs/19_license_register_index_21_countries.md '当局公式' && echo 1 || echo 0)"

# L10-L11: 21カ国インデックスの最低整合
country_count=$(awk 'BEGIN{count=0} /^\|[ ]*[0-9]+[ ]*\|/{count++} END{print count}' docs/19_license_register_index_21_countries.md)
...
https_count=$(rg -o 'https://[^ )|]+' docs/19_license_register_index_21_countries.md | wc -l | tr -d ' ')
...
check "L19" "Final architecture compendium exists" "$(required_file docs/26_fin_os_final_architecture_compendium.md && echo 1 || echo 0)"
