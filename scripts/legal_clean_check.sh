...
check "L5" "18-country official index exists" "$(required_file docs/19_license_register_index_18_countries.md && echo 1 || echo 0)"
...
# L10-L11: 18カ国インデックスの最低整合
country_count=$(awk 'BEGIN{count=0} /^\|[ ]*[0-9]+[ ]*\|/{count++} END{print count}' docs/19_license_register_index_18_countries.md)
if [[ "${country_count}" -eq 18 ]]; then
  check "L10" "18-country rows are present" "1"
else
  check "L10" "18-country rows are present" "0"
fi

https_count=$(rg -o 'https://[^ )|]+' docs/19_license_register_index_18_countries.md | wc -l | tr -d ' ')
if [[ "${https_count}" -ge 18 ]]; then
  check "L11" "Official HTTPS source links are present" "1"
else
  check "L11" "Official HTTPS source links are present" "0"
fi
