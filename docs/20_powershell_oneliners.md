...
## 5) Strict Gate（本番判定）
```powershell
Set-Location C:\Users\aiktn\fin-os-prod; if (!(Test-Path .\.golive.env)) { Copy-Item .\.golive.env.example .\.golive.env }; make check-go-live
```
...
## 9) 18カ国当局索引 + 法務クリーン + Advisory を連続実行
```powershell
