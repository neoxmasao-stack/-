...
```powershell
Set-Location "C:\Users\aiktn\fin-os-prod"; .\scripts\official_go_check.ps1
```

切り分け一発コマンド（build と release truth を再生成）:
```powershell
Set-Location "C:\Users\aiktn\fin-os-prod"; pwsh -NoProfile -File ".\scripts\build-verify.ps1"; Get-Content .\ops-evidence\status\build-state.json; pwsh -NoProfile -File ".\scripts\release-truth-refresh.ps1"; Get-Content .\ops-evidence\status\release-truth-state.json
```

## 17) FIN-OS 最終統合アーキテクチャ確認
```powershell
Set-Location C:\Users\aiktn\Documents\Codex\2026-04-23-cloud; Get-Content .\docs\26_fin_os_final_architecture_compendium.md
```

## 18) NO_GO 切り分け（Web build + release truth）
```powershell
Set-Location "C:\Users\aiktn\fin-os-prod"; npm run build
```

```powershell
Set-Location "C:\Users\aiktn\fin-os-prod"; pwsh -NoProfile -File ".\scripts\release-truth-refresh.ps1"; Get-Content .\ops-evidence\status\release-truth-state.json
```
